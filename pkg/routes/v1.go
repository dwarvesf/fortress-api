...
func loadV1Routes(r *gin.Engine, h *handler.Handler, repo store.DBRepo, s *store.Store, cfg *config.Config) {
	...
	cronjob := r.Group("/cronjobs")
	{
		...
		cronjob.POST("/init-task-order-logs", conditionalAuthMW, conditionalPermMW(model.PermissionCronjobExecute), h.Notion.InitTaskOrderLogs)
		...
		cronjob.POST("/process-invoice-emails", conditionalAuthMW, conditionalPermMW(model.PermissionCronjobExecute), h.InvoiceEmail.ProcessInvoiceEmails)
		cronjob.POST("/clone-monthly-invoices", conditionalAuthMW, conditionalPermMW(model.PermissionCronjobExecute), h.Notion.CloneMonthlyInvoices)
	}
	...
	webhook := r.Group("/webhooks")
	{
		...
		webhook.POST("/nocodb/expense", h.Webhook.HandleNocoExpense)
		...
		webhook.POST("/notion/timesheet", h.Webhook.HandleNotionTimesheet)
		webhook.POST("/notion/invoice/generate", h.Webhook.HandleNotionInvoiceGenerate)
		webhook.POST("/notion/invoice/send", h.Webhook.HandleNotionInvoiceSend)
	...
	invoiceGroup := v1.Group("/invoices")
	{
		...
		invoiceGroup.POST("/contractor/generate", conditionalAuthMW, conditionalPermMW(model.PermissionContractorInvoiceCreate), h.Invoice.GenerateContractorInvoice)
		invoiceGroup.POST("/mark-paid", conditionalAuthMW, conditionalPermMW(model.PermissionInvoiceEdit), h.Invoice.MarkPaid)
		invoiceGroup.POST("/generate-splits", conditionalAuthMW, conditionalPermMW(model.PermissionInvoiceEdit), h.Invoice.GenerateSplits)
	}
	...
	notion := v1.Group("/notion")
	{
		...
		{
			contractorPayouts.POST("/sync", conditionalAuthMW, conditionalPermMW(model.PermissionNotionCreate), h.Notion.SyncPayouts)
		}
	}
