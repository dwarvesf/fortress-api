...
func loadV1Routes(r *gin.Engine, h *handler.Handler, repo store.DBRepo, s *store.Store, cfg *config.Config) {
	...
	feedbackGroup := v1.Group("/feedbacks")
	{
		...
		feedbackGroup.POST("/:id/topics/:topicID/submit", amw.WithAuth, pmw.WithPerm("employeeEventQuestions.create"), h.Feedback.Submit)
	}
	...
	surveyGroup := v1.Group("/surveys")
	{
		...
		surveyGroup.POST("/:id/send", pmw.WithPerm("surveys.create"), h.Feedback.SendPerformmentReview)
	}
