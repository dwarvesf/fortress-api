...
func (h *handler) TransferRequest(c *gin.Context) {
	...
	if req.Status != RequestStatus || req.TokenName != TokenName {
	...
	swapTx, err := h.controller.Swap.Swap(&transferRequest)
	if err != nil {
		revertStatus := model.RevertRequestStatusSuccess
		errRevertIcy := h.controller.Swap.RevertIcyToUser(&transferRequest)
		if errRevertIcy != nil {
