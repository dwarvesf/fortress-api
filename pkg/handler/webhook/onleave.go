...
func parseOnLeaveDataFromMessage(msg model.BasecampWebhookMessage) (OnLeaveData, error) {
	...
	data.OffType = strings.TrimSpace(split[1])
...
func (h *handler) handleOnLeaveValidation(msg model.BasecampWebhookMessage) error {
	...
	commentResult := func(result, msgType string) error {
		msg := h.service.Basecamp.BuildCommentMessage(projectID, recordingID, result, msgType)
		return h.service.Basecamp.Comment.Create(projectID, recordingID, msg.Payload)
	}
	...
	if err != nil {
		...
		err := commentResult(errMsg, bcModel.CommentMsgTypeFailed)
		return err
	}

	err = commentResult("Your format looks good to go 👍", bcModel.CommentMsgTypeCompleted)
	return err
}
...
func (h *handler) validateOnLeaveData(msg model.BasecampWebhookMessage) error {
	...
	split := strings.Split(msg.Recording.Title, "|")
	if len(split) < 3 {
		return fmt.Errorf("invalid onleave title format: %v", msg.Recording.Title)
	}

	// Validate off type
	offtype := strings.ToLower(data.OffType)
	if offtype != "off" && offtype != "remote" {
		return fmt.Errorf("invalid off type for title: %v (%v needs to be off or remote)", msg.Recording.Title, offtype)
	}

	// Validate time range
	startDate := data.StartDate
	dateRange := 1 // the number of leave days
	if time.Until(startDate).Hours() < 0 {
		return fmt.Errorf("onleave start date cannot be in the past: %v", timeutil.ParseTimeToDateFormat(&startDate))
	}
	...
	if !data.EndDate.IsZero() {
		...
		if dateRange <= 1 {
			return fmt.Errorf("end date must be greater than start date: start date is %v - end date is %v", timeutil.ParseTimeToDateFormat(&startDate), timeutil.ParseTimeToDateFormat(&endDate))
		}
	}
