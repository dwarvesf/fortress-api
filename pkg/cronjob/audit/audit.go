...
func (c *cronjob) SyncAuditCycle() {
	...
	database, err := c.service.Notion.GetDatabase(AuditCycleBDID, nil)
	if err != nil {
...
func (c *cronjob) createAudit(page *notion.Page, auditCycle *model.AuditCycle) error {
	...
	auditChecklist, err := c.service.Notion.GetDatabase(audit.Results[auditChecklistIndex].ID(), nil)
	if err != nil {
		c.logger.Error(err, "failed to get database from notion")
		return err
	}
...
func (c *cronjob) getFlag(page *notion.Page, row *notion.Page) (model.AuditFlag, error) {
	...
	checklistDatabase, err := c.service.Notion.GetDatabase(checklistPage.Results[checklistDatabaseIndex].ID(), nil)
	if err != nil {
		c.logger.Error(err, "failed to get database from notion")
		return "", err
	}
...
func (c *cronjob) SyncActionItem() {
	...
	database, err := c.service.Notion.GetDatabase(AuditActionBDID, nil)
	if err != nil {
		c.logger.Error(err, "failed to get database from notion")
		return
	}
