...
func (c *controller) Swap(transferRequest *model.TransferRequestResponse) (string, error) {
	err := c.WithdrawFromVault(transferRequest)
	if err != nil {
		c.logger.Error(err, "failed to withdraw ICY")
		return "", fmt.Errorf("can't withdraw ICY, err: %v", err)
	}
...
func (c *controller) RevertIcyToUser(transferRequest *model.TransferRequestResponse) (string, error) {
	...
	if err != nil || len(transaction) == 0 {
		c.logger.Error(err, "failed to revert icy to user")
		return transferResponse.TxHash, fmt.Errorf("can't revert icy to user, err: %v", err)
	}
