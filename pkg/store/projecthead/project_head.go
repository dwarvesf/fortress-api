...
func (s *store) GetActiveLeadsByProjectID(db *gorm.DB, projectID string) ([]*model.ProjectHead, error) {
	...
	return projectHeads, db.Where("project_id = ? AND (end_date IS NULL OR end_date > now()) AND deleted_at IS NULL", projectID).
		Preload("Employee").
...
func (s *store) One(db *gorm.DB, projectID string, position model.HeadPosition) (*model.ProjectHead, error) {
	...
	return projectHead, db.Where("project_id = ? AND position = ? AND end_date IS NULL", projectID, position).First(&projectHead).Error
}

func (s *store) UpdateEndDateOfEmployee(db *gorm.DB, employeeID string, projectID string, position string, endDate time.Time) (*model.ProjectHead, error) {
	head := model.ProjectHead{}
	return &head, db.
		Model(&head).
		Where("employee_id = ? AND project_id = ? AND position = ? AND end_date IS NULL", employeeID, projectID, position).
