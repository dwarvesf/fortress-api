...
func New(cfg *config.Config, store *store.Store, repo store.DBRepo) *Service {
	...
	baseTestnetClient, err := evm.New(evm.TestnetBASEClient, cfg, logger.L)
	if err != nil {
		logger.L.Error(err, "failed to init base testnet client service")
	}
	icyBackend, err := icybackend.New(baseTestnetClient, cfg, logger.L)
	if err != nil {
