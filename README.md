...
Task provider migrations rely on the unified `TASK_PROVIDER` switch alongside the flow-specific Basecamp/Noco settings (e.g., `ACCOUNTING_BASECAMP_*`, `NOCO_ACCOUNTING_*`, `NOCO_EXPENSE_*`). Use `.env.sample` as the source of truth for the additional variables required when enabling invoice, accounting, or expense providers.
